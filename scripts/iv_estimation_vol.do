capture log close
log using geo_educ_civic_vol.log, replace

////////////////////////////////////////////////////////////////////////////////
//
// <FILE> iv_estimation_vol.do
...
// recode volunteer frequency to never if volreason != 3
capture confirm variable volfreqmod
if _rc {
    gen volfreqmod = volfreq
    recode volfreqmod (1=0) (2=1) (3=2) (4=3)
    replace volfreqmod = 0 if volreasn != 3 & !missing(volfreqmod)
}

// recode donate amount to 0 if donate == 0
replace donateamt = 0 if donate == 0

// set locals: years
local yearlist 2005 2007 2011 2013

// set locals: outcome variables
local yvars volfreqmod donate donateamt

// set locals: primary independent variable
local x ccol

// set locals: control variables
local controls asvab black hispanic multiracial female south smsa i.quarter
...
foreach y of local yvars {
    ...
    drop _all
    svmat `y'_fsmat
    outsheet using ${outdir}`y'_fsmat.csv, c replace

    drop _all
    svmat `y'_ssmat
    outsheet using ${outdir}`y'_ssmat.csv, c replace
